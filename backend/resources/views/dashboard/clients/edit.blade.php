...
@section('page-title', 'تعديل: ' . $client->name)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="locale" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="ar" {{ old('locale', $client->locale) === 'ar' ? 'selected' : '' }}>العربية</option>
                        <option value="en" {{ old('locale', $client->locale) === 'en' ? 'selected' : '' }}>English</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">الحالة</label>
                    <select name="is_active" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="1" {{ $client->is_active ? 'selected' : '' }}>نشط</option>
                        <option value="0" {{ !$client->is_active ? 'selected' : '' }}>غير نشط</option>
                    </select>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">ملاحظات داخلية</label>
                    <textarea name="notes" rows="3"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('notes', $client->notes) }}</textarea>
                </div>
            </div>
