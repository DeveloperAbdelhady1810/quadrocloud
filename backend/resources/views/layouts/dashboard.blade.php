    ...
    <aside class="sidebar bg-indigo-900 text-white fixed top-0 right-0 flex flex-col shadow-xl" id="sidebar">
        ...
        <nav class="flex-1 p-4 space-y-1">
            ...
            <a href="{{ route('dashboard.clients.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-indigo-800 {{ request()->routeIs('dashboard.clients.*') ? 'bg-indigo-700' : '' }}">
                ...
                العملاء
                @php $visReqCount = \App\Models\Client::whereNotNull('visibility_requested_at')->count(); @endphp
                @if($visReqCount > 0)
                    <span class="mr-auto bg-yellow-500 text-white text-xs rounded-full px-2 py-0.5">{{ $visReqCount }}</span>
                @endif
            </a>
            <a href="{{ route('dashboard.invoices.create') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-indigo-800 {{ request()->routeIs('dashboard.invoices.create') ? 'bg-indigo-700' : '' }}">
            ...
            <a href="{{ route('dashboard.notifications.create') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-indigo-800 {{ request()->routeIs('dashboard.notifications.*') ? 'bg-indigo-700' : '' }}">
